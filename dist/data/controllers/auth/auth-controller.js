...
const createAndSendToken = async (user, res) => {
	...
	const accessTokenCookiesOption = {
		expires: new Date(Date.now() + oneHour),
		httpOnly: true,
		secure: isSecure,
		sameSite: "none",
	};
	const refreshTokenCookiesOption = {
 ...
 */
export const verifyRefreshToken = async (refreshToken, secret, next) => {
	try {
...
export const verifyAccessToken = async (accessToken, secret, next) => {
	try {
		const verify = promisify(jwt.verify);
...
export const createNewToken = catchAsync(async (req, res, next) => {
	const body = req.body;
...
export const login = catchAsync(async (req, res, next) => {
	...
	if (!user.emailVerified) {
		...
		return next(
			new AppError("Please verify your email", HttpStatusCode.FORBIDDEN),
		);
	}
	await createAndSendToken(user, res);
});
export const protect = catchAsync(async (req, res, next) => {
	...
	if (headers.authorization && headers.authorization.startsWith("Bearer")) {
		...
		return next(
			new AppError("You are not logged in", HttpStatusCode.UNAUTHORIZED),
		);
	}
...
export const forgotPasword = catchAsync(async (req, res, next) => {
	const user = await User.findOne({
...
export const resetPasswordView = catchAsync(async (req, res) => {
	const resetToken = req.params.token;
	if (!resetToken) {
...
export const resetPasswordWeb = catchAsync(async (req, res) => {
 ...
 */
export const firebaseSolicalLogin = async (req, res, next) => {
	try {
...
export const firebasePhoneLogin = async (req, res, next) => {
	try {
		const token = req.body.token;
		if (!token) {
			return next(
...
export const passwordResetSuccess = (req, res) => {
...
export const passwordResetFailure = (req, res) => {
	const token = req.query.token;
	if (!token) {
		return res.render("page/404");
	}
